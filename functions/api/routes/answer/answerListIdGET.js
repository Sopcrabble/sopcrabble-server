...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const answer = await answerDB.getAllAnswers(client, id);
    const data = {
      "questionId" : answer[0].questionId,
      "title" : answer[0].title,
      "wordNum" : answer[0].wordNum,
      "answers" : answer
    };
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_ALL_ANSWER_SUCCESS, data));
  } catch (error) {
